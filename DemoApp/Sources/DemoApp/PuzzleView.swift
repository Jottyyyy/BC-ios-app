...
struct PuzzleView: View {
    ...
    private var header: some View {
        HStack(spacing: 14) {
            Text("Puzzles").font(.largeTitle.weight(.bold))
            Spacer()
    ...
    private func badge(_ label: String, _ value: String, _ tint: Color) -> some View {
        VStack(spacing: 1) {
            Text(value).font(.system(.headline, design: .rounded)).foregroundStyle(tint)
        ...
        HStack(spacing: 4) {
            VStack(spacing: 0) {
                ForEach(ranks(), id: \.self) { r in
                    Text("\(r + 1)").font(.caption2).foregroundStyle(.secondary).frame(width: 14, height: square)
                }
            }
            VStack(spacing: 4) {
                ...
                HStack(spacing: 0) {
                    ForEach(files(), id: \.self) { f in
                        Text(String(UnicodeScalar(UInt8(97 + f)))).font(.caption2).foregroundStyle(.secondary).frame(width: square)
                    }
                }
            }
        }
    ...
    private var statusRow: some View {
        HStack(spacing: 12) {
            ...
            if vm.phase == .failed, let san = vm.correctSAN {
                Text("Best: \(san)").font(.callout.monospaced()).foregroundStyle(.secondary)
            }
            Spacer()
            if vm.phase == .solving {
        ...
        VStack(alignment: .leading, spacing: 12) {
            Card(title: "This puzzle") {
                VStack(alignment: .leading, spacing: 8) {
                    if let p = vm.puzzle {
                        Text("\(vm.sideToMove == .white ? "White" : "Black") to move").font(.headline)
                        Text("Rating \(p.rating)").font(.caption).foregroundStyle(.secondary)
                        if !p.themes.isEmpty {
                            FlowChips(items: Array(p.themes.prefix(6)))
                        }
                    } else {
                        Text(vm.message).font(.caption).foregroundStyle(.secondary)
                    }
                }
            }
            ...
            HStack(spacing: 10) {
                ForEach([PieceKind.queen, .rook, .bishop, .knight], id: \.self) { k in
                    Button { vm.choosePromotion(k) } label: {
                        Text(BoardView.glyph(Piece(color, k))).font(.system(size: 34))
