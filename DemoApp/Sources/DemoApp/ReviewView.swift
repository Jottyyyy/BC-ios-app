    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 16) {
                Text("Game Review").font(.largeTitle.weight(.bold))
                ...
                Card(title: "Classify one move") {
                    VStack(spacing: 14) {
                        ...
                        HStack {
                            ...
                            Text("(a capture 'x' with ≥150 gain = Brilliant)").font(.caption).foregroundStyle(.secondary)
                            Spacer()
                        }
                    }
                }
        ...
        Card(title: "Full sample game review") {
            VStack(alignment: .leading, spacing: 10) {
                ...
                ForEach(result.moveEvaluations, id: \.moveIndex) { m in
                    HStack {
                        Text("Move \(m.moveIndex)").frame(width: 70, alignment: .leading).foregroundStyle(.secondary)
                        Text(m.classification.capitalized).fontWeight(.semibold)
                        Spacer()
                        Text("cp loss \(m.cpLoss)").monospacedDigit().foregroundStyle(.secondary)
                    }
                    .font(.callout)
                }
            }
