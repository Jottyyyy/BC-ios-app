    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 16) {
                Text("Tournament").font(.largeTitle.weight(.bold))
                ...
                Card(title: "Setup") {
                    VStack(spacing: 12) {
                        ...
                        HStack {
                            ...
                            if vm.type == "swiss" {
                                Spacer(); Text("Rounds").foregroundStyle(.secondary)
                ...
                if vm.started {
                    HStack {
                        Text(vm.message).foregroundStyle(.secondary)
                        Spacer()
        ...
        Card(title: "Round \(vm.completedRounds + 1) pairings") {
            VStack(spacing: 8) {
                ForEach(Array(vm.currentPairings.enumerated()), id: \.offset) { idx, p in
                    HStack {
                        Text("Bd \(idx + 1)").frame(width: 44, alignment: .leading).foregroundStyle(.secondary)
                        if p.isBye {
        ...
        Card(title: "Standings") {
            VStack(spacing: 6) {
                ...
                ForEach(Array(vm.standings().enumerated()), id: \.element.id) { i, p in
                    HStack {
                        Text("\(i + 1)").frame(width: 24, alignment: .leading).foregroundStyle(.secondary)
                        ...
                        Text(fmt(p.directEncounter)).frame(width: 44, alignment: .trailing).monospacedDigit().foregroundStyle(.secondary)
                        Text("\(p.buchholz)").frame(width: 52, alignment: .trailing).monospacedDigit().foregroundStyle(.secondary)
                        Text("\(p.sonnebornBerger)").frame(width: 44, alignment: .trailing).monospacedDigit().foregroundStyle(.secondary)
                        Text("\(p.wins)-\(p.draws)-\(p.losses)").frame(width: 70, alignment: .trailing).monospacedDigit().foregroundStyle(.secondary)
                    }
                    .font(.callout)
                }
            }
